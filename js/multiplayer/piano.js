...
(function initPapianoOnline(){
    ...
    function attachBlocksListener(){
        ...
        async function refresh(){
            if(stopped) return;
            try{
                const data = await window.papianoData.gql(`query { listMyBlocks { blockerId blockedId } }`);
                blockedPlayerIds.clear();
                (data?.listMyBlocks || []).forEach(b => { if(b.blockedId) blockedPlayerIds.add(String(b.blockedId)); });
                renderMessages();
                renderRoomChrome();
    ...
    async function toggleBlockedPlayer(player){
        ...
        if(!mpSelfId || isLocalGuestPlayerId(mpSelfId)){
        ...
        try{
            if(blocked){
                await window.papianoData.gql(`mutation($id: String!) { unblockUser(blockedId: $id) }`, { id });
                blockedPlayerIds.delete(id);
                showToast('Player unblocked.', { type:'success', title:'Profile' });
            }else{
                await window.papianoData.gql(`mutation($id: String!) { blockUser(blockedId: $id) { blockedId } }`, { id });
    ...
    async function toggleFriend(player){
        ...
        try{
            await window.papianoData.gql(`mutation($u: String!) { sendFriendRequest(otherUid: $u) { status } }`, { u: id });
            showToast('Friend request sent.', { type:'success', title:'Friends' });
            syncFriendAction(player);
        }catch(e){
            if(e.errorType === 'Conflict') showToast('Already friends or request already sent.', { type:'info', title:'Friends' });
            else showToast('Couldn’t send friend request. Try again.', { type:'error', title:'Friends' });
        }
    }
    ...
    async function loadProfileVote(player){
        ...
        try{
            const data = await window.papianoData.gql(`query($p: ID!) { myProfileReaction(profileUid: $p) }`, { p: player.id });
            const type = String(data?.myProfileReaction || '');
    ...
    async function submitPlayerReport(){
        ...
        try{
            await window.papianoData.gql(
                `mutation($input: SubmitReportInput!) { submitReport(input: $input) { targetId } }`,
                { input: {
                    targetId: player.id,
    ...
    async function castProfileVote(voteType){
        ...
        try{
            const reactionData = await window.papianoData.gql(`query($p: ID!) { myProfileReaction(profileUid: $p) }`, { p: player.id });
            ...
            const data = await window.papianoData.gql(
                ...
                { u: player.id, t: nextType }
            );
            const result = { likes: safeReactionCount(data?.voteProfile?.likes), dislikes: safeReactionCount(data?.voteProfile?.dislikes), vote: nextType === 'none' ? '' : nextType };
