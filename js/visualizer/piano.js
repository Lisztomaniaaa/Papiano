...
(function initPapianoOnline(){
    ...
    // engine in solo mode (no backend room/chat); the multiplayer room/chat
...
if(!window.PAPIANO_VISUALIZER) return;

/* ── MIDI Parser (format 0 & 1) ───────────────────────────────── */
function parseMidi(arrayBuffer){
    ...
    function readVarLen(){let val=0,b;do{b=data[pos++];val=(val<<7)|(b&0x7f);}while(b&0x80);return val;}
    ...
    for(let t=0;t<numTracks;t++){
        ...
        while(pos<trackEnd){
            ...
            if(statusByte&0x80){pos++;if(statusByte<0xf0)runningStatus=statusByte;}
            else statusByte=runningStatus;
            const type=statusByte&0xf0;
            if(statusByte===0xff){const metaType=data[pos++];const metaLen=readVarLen();if(metaType===0x51){const tempo=(data[pos]<<16)|(data[pos+1]<<8)|data[pos+2];events.push({delta,type:'tempo',tempo});}pos+=metaLen;}
            else if(statusByte===0xf0||statusByte===0xf7){const sysLen=readVarLen();pos+=sysLen;}
            else if(type===0x90){const note=data[pos++];const vel=data[pos++];events.push({delta,type:vel>0?'noteOn':'noteOff',note,velocity:vel/127});}
            else if(type===0x80){const note=data[pos++];pos++;events.push({delta,type:'noteOff',note,velocity:0});}
...
function midiToTimeline(parsed){
    ...
    const tempoMap=[];
    for(const track of parsed.tracks){let tick=0;for(const ev of track){tick+=ev.delta;if(ev.type==='tempo')tempoMap.push({tick,tempo:ev.tempo});}}
    tempoMap.sort((a,b)=>a.tick-b.tick);
    if(tempoMap.length===0)tempoMap.push({tick:0,tempo:500000});
    function tickToMs(targetTick){let cTick=0,cMs=0,cTempo=500000,idx=0;while(idx<tempoMap.length&&tempoMap[idx].tick<=targetTick){const t=tempoMap[idx];cMs+=(t.tick-cTick)*(cTempo/ticksPerBeat)/1000;cTick=t.tick;cTempo=t.tempo;idx++;}cMs+=(targetTick-cTick)*(cTempo/ticksPerBeat)/1000;return cMs;}
    const timeline=[];
    for(const track of parsed.tracks){let tick=0;for(const ev of track){tick+=ev.delta;if(ev.type==='noteOn'||ev.type==='noteOff')timeline.push({time:tick,ms:tickToMs(tick),type:ev.type,note:ev.note,velocity:ev.velocity});}}
...
function loadVizTimeline(timeline,name){
    ...
    vizLoaded=true;
    updateBtnLabel();
    if(typeof showToast==='function')showToast('Loaded: '+(name||'file')+' — press Play to start',{type:'success'});
}
...
function vizTick(){
    if(!vizPlaying)return;
    const now=performance.now();
    vizElapsedMs+=now-vizLastTickAt;
    vizLastTickAt=now;
