// Writes build version to version.json and stamps CSS/JS URLs (runs on Amplify build — see amplify.yml)
const fs = require('fs');
...
// Stamp cache-bust in every HTML entry served by Amplify.
['index.html', 'multiplayer.html', 'solo.html', 'visualizer.html', 'visualizer-stage.html', 'admin.html'].forEach(file => {
  if (!fs.existsSync(file)) return;
  let html = fs.readFileSync(file, 'utf8');
  // Cache-bust every versioned asset. Paths are folder-specific so each
  // area (app / solo / multiplayer / visualizer / shared) is stamped unambiguously.
  ...
  html = html.replace(/(app\/cognito-auth\.js)\?v=[^"']*/g, '$1?v=' + version);
  html = html.replace(/(app\/appsync-client\.js)\?v=[^"']*/g, '$1?v=' + version);
