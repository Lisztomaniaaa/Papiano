/*
 * /api/storage-presign — issues a short-lived presigned S3 PUT URL for
 * avatar and chat-image uploads. The browser PUTs the file straight to S3
 * (not through this function) to stay under serverless body-size limits.
 *
 *   POST { idToken, bucket: 'avatars'|'chat-images', folder, fileName, contentType }
 *   -> 200 { ok:true, uploadUrl, publicUrl, key }
 *   -> 4xx/5xx { ok:false, reason }
 *
 * The uid embedded in the object key always comes from the verified
 * idToken, never from client input, so one user can't write into another
 * user's prefix.
 */
